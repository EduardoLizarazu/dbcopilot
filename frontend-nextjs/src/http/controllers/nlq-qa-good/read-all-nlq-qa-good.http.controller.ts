...
import { IReadAllNlqQaGoodUseCase } from "@/core/application/usecases/nlq/nlq-qa-good/read-all-nlq-qa-good.usecase";

export class ReadAllNlqQaGoodController implements IController {
  constructor(
    private readonly logger: ILogger,
    private readonly readAllNlqQaGoodUseCase: IReadAllNlqQaGoodUseCase,
  ...
  async handle(httpRequest: IHttpRequest<null>): Promise<IHttpResponse> {
    try {
      // ==== INPUT OF REQUEST ====
      this.logger.info(
        "[ReadAllNlqQaGoodController] handling request",
        httpRequest
      );
      ...
      this.logger.info("[ReadAllNlqQaGoodController] Headers:", headers);
      //   2. Check authorization
      const authHeader =
        headers["Authorization"] || headers["authorization"] || "";
      if (!authHeader.startsWith("Bearer ")) {
        this.logger.error(
          "[ReadAllNlqQaGoodController] No token provided",
          httpRequest
        );
      ...
      this.logger.info("[ReadAllNlqQaGoodController] Token:", token);

      //   3. Decode token
      const decoded = await this.decodeTokenAdapter.decodeToken(token);
      if (!decoded) {
        this.logger.error(
          "[ReadAllNlqQaGoodController] Invalid token",
          httpRequest
        );
      ...
      this.logger.info(
        "[ReadAllNlqQaGoodController] User roles names:",
        roleNames.roleNames
      );
      ...
      if (!hasAuth) {
        this.logger.error(
          "[ReadAllNlqQaGoodController] User is not authorized"
        );
      ...
      this.logger.info(
        "[ReadAllNlqQaGoodController] UseCase executed successfully",
        useCase
      );

      if (!useCase.success) {
        this.logger.error("[ReadAllNlqQaGoodController] Error creating", {
          ...useCase,
        });
      ...
      this.logger.error("[ReadAllNlqQaGoodController] Error:", error);
