...
import { IDeleteQaGoodUseCase } from "@/core/application/usecases/nlq/nlq-qa-good/delete-qa-good.usecase";

export class DeleteNlqQaGoodByIdController implements IController {
  constructor(
    private readonly logger: ILogger,
    private readonly deleteNlqQaGoodByIdUseCase: IDeleteQaGoodUseCase,
  ...
  async handle(httpRequest: IHttpRequest<null>): Promise<IHttpResponse> {
    try {
      // ==== INPUT OF REQUEST ====
      this.logger.info(
        "[DeleteNlqQaGoodByIdController] handling request",
        httpRequest
      );
      ...
      this.logger.info("[DeleteNlqQaGoodByIdController] Headers:", headers);
      //   2. Check authorization
      const authHeader =
        headers["Authorization"] || headers["authorization"] || "";
      if (!authHeader.startsWith("Bearer ")) {
        this.logger.error(
          "[DeleteNlqQaGoodByIdController] No token provided",
          httpRequest
        );
      ...
      this.logger.info("[DeleteNlqQaGoodByIdController] Token:", token);

      //   3. Decode token
      const decoded = await this.decodeTokenAdapter.decodeToken(token);
      if (!decoded) {
        this.logger.error(
          "[DeleteNlqQaGoodByIdController] Invalid token",
          httpRequest
        );
      ...
      this.logger.info(
        "[DeleteNlqQaGoodByIdController] User roles names:",
        roleNames.roleNames
      );
      ...
      if (!hasAuth) {
        this.logger.error(
          "[DeleteNlqQaGoodByIdController] User is not authorized"
        );
      ...
      if (!httpRequest.params || !httpRequest.params.id) {
        this.logger.error(
          "[DeleteNlqQaGoodByIdController] No id parameter provided",
          httpRequest
        );
      ...
      this.logger.info(
        "[DeleteNlqQaGoodByIdController] Received params:",
        httpRequest.params
      );

      // ==== BUSINESS LOGIC USE CASES ====
      const useCase = await this.deleteNlqQaGoodByIdUseCase.execute(
        httpRequest.params.id
      );
      this.logger.info(
        "[DeleteNlqQaGoodByIdController] UseCase executed successfully",
        useCase
      );

      if (!useCase.success) {
        this.logger.error("[DeleteNlqQaGoodByIdController] Error creating", {
          ...useCase,
        });
      ...
      this.logger.error("[DeleteNlqQaGoodByIdController] Error:", error);
