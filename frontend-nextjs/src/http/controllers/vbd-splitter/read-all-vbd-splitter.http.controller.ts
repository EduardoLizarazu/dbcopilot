...
import { IReadAllVbdSplitterUseCase } from "@/core/application/usecases/vbd-splitter/read-all-vbd-splitter.usecase";

export class ReadAllVbdSplitterController implements IController {
  constructor(
    private readonly logger: ILogger,
    private readonly readAllVbdSplitterUseCase: IReadAllVbdSplitterUseCase,
  ...
  async handle(httpRequest: IHttpRequest<null>): Promise<IHttpResponse> {
    try {
      // ==== INPUT OF REQUEST ====
      this.logger.info(
        "[ReadAllVbdSplitterController] handling request",
        httpRequest
      );
      ...
      this.logger.info("[ReadAllVbdSplitterController] Headers:", headers);
      //   2. Check authorization
      const authHeader =
        headers["Authorization"] || headers["authorization"] || "";
      if (!authHeader.startsWith("Bearer ")) {
        this.logger.error(
          "[ReadAllVbdSplitterController] No token provided",
          httpRequest
        );
      ...
      this.logger.info("[ReadAllVbdSplitterController] Token:", token);

      //   3. Decode token
      const decoded = await this.decodeTokenAdapter.decodeToken(token);
      if (!decoded) {
        this.logger.error(
          "[ReadAllVbdSplitterController] Invalid token",
          httpRequest
        );
      ...
      this.logger.info(
        "[ReadAllVbdSplitterController] User roles names:",
        roleNames.roleNames
      );
      ...
      if (!hasAuth) {
        this.logger.error(
          "[ReadAllVbdSplitterController] User is not authorized"
        );
      ...
      const useCase = await this.readAllVbdSplitterUseCase.execute();
      this.logger.info(
        "[ReadAllVbdSplitterController] UseCase executed successfully",
        useCase
      );

      if (!useCase.success) {
        this.logger.error(
          "[ReadAllVbdSplitterController] Error reading VBD Splitters",
          {
            ...useCase,
          }
        );
        const error = this.httpErrors.error_400(
          "Error reading VBD Splitters: " + useCase.message
        );
        return new HttpResponse(error.statusCode, error.body);
      }
      ...
      const success = this.httpSuccess.success_201({
        message: "VBD Splitters read successfully",
        data: useCase.data,
      });
      return new HttpResponse(success.statusCode, success.body);
    } catch (error) {
      this.logger.error(
        "[ReadAllVbdSplitterController] Unexpected error",
        error
      );
