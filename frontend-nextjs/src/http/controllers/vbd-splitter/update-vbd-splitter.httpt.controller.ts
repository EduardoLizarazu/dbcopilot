...
export class UpdateVbdSplitterController implements IController {
  ...
  async handle(
    ...
    try {
      // ==== INPUT OF REQUEST ====
      this.logger.info(
        "[ReadVbdSplitterByIdController] handling request",
        httpRequest
      );
      ...
      this.logger.info("[ReadVbdSplitterByIdController] Headers:", headers);
      //   2. Check authorization
      const authHeader =
        headers["Authorization"] || headers["authorization"] || "";
      if (!authHeader.startsWith("Bearer ")) {
        this.logger.error(
          "[ReadVbdSplitterByIdController] No token provided",
          httpRequest
        );
        const error = this.httpErrors.error_400(
          "Error retrieving VBD Splitter"
        );
        return new HttpResponse(error.statusCode, error.body);
      }
      const token = authHeader.replace("Bearer ", "");
      this.logger.info("[ReadVbdSplitterByIdController] Token:", token);

      //   3. Decode token
      const decoded = await this.decodeTokenAdapter.decodeToken(token);
      if (!decoded) {
        this.logger.error(
          "[ReadVbdSplitterByIdController] Invalid token",
          httpRequest
        );
      ...
      this.logger.info(
        "[ReadVbdSplitterByIdController] User roles names:",
        roleNames.roleNames
      );
      ...
      if (!hasAuth) {
        this.logger.error(
          "[ReadVbdSplitterByIdController] User is not authorized"
        );
      ...
      if (!httpRequest.params || !httpRequest.params.id) {
        this.logger.error(
          "[ReadVbdSplitterByIdController] No id parameter provided",
          httpRequest
        );
      ...
      this.logger.info(
        "[ReadVbdSplitterByIdController] Received params:",
        httpRequest.params
      );
      ...
      if (!useCase.success) {
        this.logger.error(
          "[ReadVbdSplitterByIdController] Error retrieving VBD Splitter",
          {
            ...useCase,
          }
        );
