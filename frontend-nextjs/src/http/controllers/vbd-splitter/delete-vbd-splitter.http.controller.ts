...
import { IDeleteVbdSplitterUseCase } from "@/core/application/usecases/vbd-splitter/delete-vbd-splitter.usecase";

export class DeleteVbdSplitterController implements IController {
  constructor(
    private readonly logger: ILogger,
    private readonly deleteVbdSplitterUseCase: IDeleteVbdSplitterUseCase,
  ...
  async handle(httpRequest: IHttpRequest<null>): Promise<IHttpResponse> {
    try {
      // ==== INPUT OF REQUEST ====
      this.logger.info(
        "[DeleteVbdSplitterController] handling request",
        httpRequest
      );
      ...
      this.logger.info("[DeleteVbdSplitterController] Headers:", headers);
      //   2. Check authorization
      const authHeader =
        headers["Authorization"] || headers["authorization"] || "";
      if (!authHeader.startsWith("Bearer ")) {
        this.logger.error(
          "[DeleteVbdSplitterController] No token provided",
          httpRequest
        );
        const error = this.httpErrors.error_400("Error deleting VBD Splitter");
        return new HttpResponse(error.statusCode, error.body);
      }
      const token = authHeader.replace("Bearer ", "");
      this.logger.info("[DeleteVbdSplitterController] Token:", token);

      //   3. Decode token
      const decoded = await this.decodeTokenAdapter.decodeToken(token);
      if (!decoded) {
        this.logger.error(
          "[DeleteVbdSplitterController] Invalid token",
          httpRequest
        );
      ...
      this.logger.info(
        "[DeleteVbdSplitterController] User roles names:",
        roleNames.roleNames
      );
      ...
      if (!hasAuth) {
        this.logger.error(
          "[DeleteVbdSplitterController] User is not authorized"
        );
      ...
      if (!httpRequest.params || !httpRequest.params.id) {
        this.logger.error(
          "[DeleteVbdSplitterController] No id parameter provided",
          httpRequest
        );
      ...
      this.logger.info(
        "[DeleteVbdSplitterController] Received params:",
        httpRequest.params
      );

      // ==== BUSINESS LOGIC USE CASES ====
      const useCase = await this.deleteVbdSplitterUseCase.execute(
        httpRequest.params.id
      );
      this.logger.info(
        "[DeleteVbdSplitterController] UseCase executed successfully",
        useCase
      );

      if (!useCase.success) {
        this.logger.error(
          "[DeleteVbdSplitterController] Error deleting VBD Splitter",
          {
            ...useCase,
          }
        );
