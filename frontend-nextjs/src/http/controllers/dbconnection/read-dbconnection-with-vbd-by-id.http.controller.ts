...
export class DeleteDbConnectionController implements IController {
  ...
  async handle(httpRequest: IHttpRequest<null>): Promise<IHttpResponse> {
    try {
      // ==== INPUT OF REQUEST ====
      this.logger.info(
        "[ReadDbConnectionWithVbdByIdController] handling request",
        httpRequest
      );
      ...
      this.logger.info(
        "[ReadDbConnectionWithVbdByIdController] Headers:",
        headers
      );
      ...
      if (!authHeader.startsWith("Bearer ")) {
        this.logger.error(
          "[ReadDbConnectionWithVbdByIdController] No token provided",
          httpRequest
        );
        const error = this.httpErrors.error_400("Error reading DB Connection");
        return new HttpResponse(error.statusCode, error.body);
      }
      const token = authHeader.replace("Bearer ", "");
      this.logger.info("[ReadDbConnectionWithVbdByIdController] Token:", token);

      //   3. Decode token
      const decoded = await this.decodeTokenAdapter.decodeToken(token);
      if (!decoded) {
        this.logger.error(
          "[ReadDbConnectionWithVbdByIdController] Invalid token",
          httpRequest
        );
      ...
      this.logger.info(
        "[ReadDbConnectionWithVbdByIdController] User roles names:",
        roleNames.roleNames
      );
      ...
      if (!hasAuth) {
        this.logger.error(
          "[ReadDbConnectionWithVbdByIdController] User is not authorized"
        );
      ...
      if (!params || !params.id) {
        this.logger.error(
          "[ReadDbConnectionWithVbdByIdController] No id parameter provided"
        );
      ...
      const useCase = await this.readDbConnectionWithVbdByIdUseCase.execute(
        params.id
      );

      if (!useCase.success) {
        this.logger.error(
          "[ReadDbConnectionWithVbdByIdController] Error reading DB Connection",
          {
            ...useCase,
          }
        );
