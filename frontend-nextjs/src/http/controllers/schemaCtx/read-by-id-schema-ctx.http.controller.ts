...
import { IReadByIdSchemaCtxUseCase } from "@/core/application/usecases/schemaCtx/read-by-id-schema-ctx.usecase";

export class ReadByIdSchemaCtxController implements IController {
  constructor(
    private readonly logger: ILogger,
    private readonly process: IReadByIdSchemaCtxUseCase,
  ...
  async handle(httpRequest: IHttpRequest<null>): Promise<IHttpResponse> {
    try {
      // ==== INPUT OF REQUEST ====
      this.logger.info(
        "[ReadByIdSchemaCtxController] handling request",
        httpRequest
      );
      ...
      this.logger.info("[ReadByIdSchemaCtxController] Headers:", headers);
      //   2. Check authorization
      const authHeader =
        headers["Authorization"] || headers["authorization"] || "";
      if (!authHeader.startsWith("Bearer ")) {
        this.logger.error(
          "[ReadByIdSchemaCtxController] No token provided",
          httpRequest
        );
        const error = this.httpErrors.error_400("Error reading schema context");
        return new HttpResponse(error.statusCode, error.body);
      }
      const token = authHeader.replace("Bearer ", "");
      this.logger.info("[ReadByIdSchemaCtxController] Token:", token);

      //   3. Decode token
      const decoded = await this.decodeTokenAdapter.decodeToken(token);
      if (!decoded) {
        this.logger.error(
          "[ReadByIdSchemaCtxController] Invalid token",
          httpRequest
        );
      ...
      this.logger.info(
        "[ReadByIdSchemaCtxController] User roles names:",
        roleNames.roleNames
      );
      ...
      if (!hasAuth) {
        this.logger.error(
          "[ReadByIdSchemaCtxController] User is not authorized"
        );
      ...
      if (!params || !params.id) {
        this.logger.error(
          "[ReadByIdSchemaCtxController] No id parameter provided"
        );
      ...
      const useCase = await this.process.execute(params.id);

      if (!useCase.success) {
        this.logger.error(
          "[ReadByIdSchemaCtxController] Error reading schema context",
          {
            ...useCase,
          }
        );
      ...
      this.logger.error(
        "[ReadByIdSchemaCtxController] Unexpected error",
        err.message
      );
