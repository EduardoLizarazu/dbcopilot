...
export class CreateSchemaCtxController implements IController {
  constructor(
    ...
    private readonly createSchemaCtxUseCase: ICreateSchemaCtxUseCase,
  ...
  async handle(
    ...
    try {
      // ==== INPUT OF REQUEST ====
      this.logger.info(
        "[CreateSchemaCtxController] Handling request",
        httpRequest
      );
      ...
      this.logger.info("[CreateSchemaCtxController] Headers:", headers);
      //   2. Check authorization
      const authHeader =
        headers["Authorization"] || headers["authorization"] || "";
      if (!authHeader.startsWith("Bearer ")) {
        this.logger.error(
          "[CreateSchemaCtxController] Authorization token is missing",
          httpRequest
        );
      ...
      this.logger.info("[CreateSchemaCtxController] Token:", token);

      //   3. Decode token
      const decoded = await this.decodeTokenAdapter.decodeToken(token);
      if (!decoded) {
        this.logger.error(
          "[CreateSchemaCtxController] Invalid authorization token",
          httpRequest
        );
      ...
      this.logger.info(
        "[CreateSchemaCtxController] User roles names:",
        roleNames.roleNames
      );
      ...
      if (!hasAuth) {
        this.logger.error(
          "[CreateSchemaCtxController] User does not have the required roles",
          httpRequest
        );
      ...
      if (!httpRequest.body) {
        this.logger.error(
          "[CreateSchemaCtxController] No body provided",
          httpRequest
        );
      ...
      this.logger.info(
        "[CreateSchemaCtxController] Received body:",
        httpRequest.body
      );

      const body = httpRequest.body;

      const response = await this.createSchemaCtxUseCase.execute(body);

      if (!response.success) {
        this.logger.error(
          "[CreateSchemaCtxController] Failed to create schema context:",
          response
        );
      ...
      this.logger.info(
        "[CreateSchemaCtxController] Created schema context:",
        response.data
      );
      ...
      this.logger.error(
        "[CreateSchemaCtxController] Unexpected error:",
        err.message
      );
