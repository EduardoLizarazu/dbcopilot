...
import { IReadNlqQaFbOrErrorByIdUseCase } from "@/core/application/usecases/nlq/nlq-qa-correction/read-nlq-fb-or-error-by-id.usecase";

export class ReadNlqQaFbOrErrorByIdController implements IController {
  constructor(
    private readonly logger: ILogger,
    private readonly readByIdRoleUseCase: IReadNlqQaFbOrErrorByIdUseCase,
  ...
  async handle(httpRequest: IHttpRequest<null>): Promise<IHttpResponse> {
    try {
      // ==== INPUT OF REQUEST ====
      this.logger.info(
        "[ReadNlqQaFbOrErrorByIdController] Handling request",
        httpRequest
      );
      ...
      this.logger.info("[ReadNlqQaFbOrErrorByIdController] Headers:", headers);
      //   2. Check authorization
      const authHeader =
        headers["Authorization"] || headers["authorization"] || "";
      if (!authHeader.startsWith("Bearer ")) {
        this.logger.error(
          "[ReadNlqQaFbOrErrorByIdController] Authorization token is missing",
          httpRequest
        );
      ...
      this.logger.info("[ReadNlqQaFbOrErrorByIdController] Token:", token);

      //   3. Decode token
      const decoded = await this.decodeTokenAdapter.decodeToken(token);
      if (!decoded) {
        this.logger.error(
          "[ReadNlqQaFbOrErrorByIdController] Invalid authorization token",
          httpRequest
        );
      ...
      this.logger.info(
        "[ReadNlqQaFbOrErrorByIdController] User roles names:",
        roleNames.roleNames
      );
      ...
      if (!hasAuth) {
        this.logger.error(
          "[ReadNlqQaFbOrErrorByIdController] User does not have the required roles",
          httpRequest
        );
      ...
      if (!httpRequest.params || !httpRequest.params.id) {
        this.logger.error(
          "[ReadNlqQaFbOrErrorByIdController] No id parameter provided",
          httpRequest
        );
      ...
      this.logger.info(
        "[ReadNlqQaFbOrErrorByIdController] Received params:",
        httpRequest.params
      );
      ...
      if (!useCase.success) {
        this.logger.error(
          "[ReadNlqQaFbOrErrorByIdController] Error retrieving: ",
          {
            ...useCase,
          }
        );
      ...
      this.logger.error(
        "[ReadNlqQaFbOrErrorByIdController] Internal server error:",
        error
      );
