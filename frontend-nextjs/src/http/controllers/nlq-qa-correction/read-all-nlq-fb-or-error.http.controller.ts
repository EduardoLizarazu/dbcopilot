...
import { IReadAllNlqQaFbOrErrorUseCase } from "@/core/application/usecases/nlq/nlq-qa-correction/read-all-nlq-fb-or-error.usecase";

export class ReadAllNlqQaFbOrErrorController implements IController {
  constructor(
    private readonly logger: ILogger,
    private readonly readAllNlqQaFbOrErrorUseCase: IReadAllNlqQaFbOrErrorUseCase,
  ...
  async handle(httpRequest: IHttpRequest<null>): Promise<IHttpResponse> {
    try {
      // ==== INPUT OF REQUEST ====q
      this.logger.info(
        "[ReadAllNlqQaFbOrErrorController] handling request",
        httpRequest
      );
      ...
      this.logger.info("[ReadAllNlqQaFbOrErrorController] Headers:", headers);
      //   2. Check authorization
      const authHeader =
        headers["Authorization"] || headers["authorization"] || "";
      if (!authHeader.startsWith("Bearer ")) {
        this.logger.error(
          "[ReadAllNlqQaFbOrErrorController] No token provided",
          httpRequest
        );
      ...
      this.logger.info("[ReadAllNlqQaFbOrErrorController] Token:", token);

      //   3. Decode token
      const decoded = await this.decodeTokenAdapter.decodeToken(token);
      if (!decoded) {
        this.logger.error(
          "[ReadAllNlqQaFbOrErrorController] Invalid token",
          httpRequest
        );
      ...
      this.logger.info(
        "[ReadAllNlqQaFbOrErrorController] User roles names:",
        roleNames.roleNames
      );
      ...
      if (!hasAuth) {
        this.logger.error(
          "[ReadAllNlqQaFbOrErrorController] User not authorized",
          httpRequest
        );
      ...
      if (!useCase.success) {
        this.logger.error(
          "[ReadAllNlqQaFbOrErrorController] Error retrieving: ",
          {
            ...useCase,
          }
        );
      ...
      this.logger.error("[ReadAllNlqQaFbOrErrorController] Error:", error);
