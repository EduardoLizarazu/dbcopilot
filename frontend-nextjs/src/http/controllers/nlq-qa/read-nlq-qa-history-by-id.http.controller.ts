...
import { IReadNlqQaHistoryByIdUseCase } from "@/core/application/usecases/nlq/nlq-qa/read-nlq-qa-history-by-id.usecase";

export class ReadNlqQaHistoryByIdController implements IController {
  constructor(
    private readonly logger: ILogger,
    private readonly readNlqQaHistoryByIdUseCase: IReadNlqQaHistoryByIdUseCase,
  ...
  async handle(
    ...
    try {
      // ==== INPUT OF REQUEST ====
      this.logger.info(
        "[ReadNlqQaHistoryByIdController] handling request",
        httpRequest
      );
      ...
      this.logger.info("[ReadNlqQaHistoryByIdController] Headers:", headers);
      //   2. Check authorization
      const authHeader =
        headers["Authorization"] || headers["authorization"] || "";
      if (!authHeader.startsWith("Bearer ")) {
        this.logger.error(
          "[ReadNlqQaHistoryByIdController] No token provided",
          httpRequest
        );
        const error = this.httpErrors.error_400(
          "Error reading NLQ history by id"
        );
        return new HttpResponse(error.statusCode, error.body);
      }
      const token = authHeader.replace("Bearer ", "");
      this.logger.info("[ReadNlqQaHistoryByIdController] Token:", token);

      //   3. Decode token
      const decoded = await this.decodeTokenAdapter.decodeToken(token);
      if (!decoded) {
        this.logger.error(
          "[ReadNlqQaHistoryByIdController] Invalid token",
          httpRequest
        );
      ...
      this.logger.info(
        "[ReadNlqQaHistoryByIdController] User roles names:",
        roleNames.roleNames
      );
      ...
      if (!hasAuth) {
        this.logger.error(
          "[ReadNlqQaHistoryByIdController] User is not authorized"
        );
      ...
      if (!httpRequest.params || !httpRequest.params.id) {
        this.logger.error(
          "[ReadNlqQaHistoryByIdController] No id parameter provided",
          httpRequest
        );
      ...
      const useCase = await this.readNlqQaHistoryByIdUseCase.execute(
        httpRequest.params.id
      );

      if (!useCase.success) {
        this.logger.error(
          "[ReadNlqQaHistoryByIdController] Error reading NLQ history by id",
          {
            ...useCase,
          }
        );
      ...
      const success = this.httpSuccess.success_200({
        message: useCase.message || "NLQ QA history item retrieved",
        data: useCase.data,
      });
      return new HttpResponse(success.statusCode, success.body);
    } catch (err) {
      this.logger.error(
        "[ReadNlqQaHistoryByIdController] Unexpected error",
        err
      );
