...
import { IToggleNlqQaIgnoreUseCase } from "@/core/application/usecases/nlq/nlq-qa/toggle-nlq-qa-ignore.usecase";

export class ToggleNlqQaIgnoreController implements IController {
  constructor(
    private readonly logger: ILogger,
    private readonly useCase: IToggleNlqQaIgnoreUseCase,
  ...
  async handle(
    ...
    try {
      // ==== INPUT OF REQUEST ====
      this.logger.info(
        "[ToggleNlqQaIgnoreController] handling request",
        httpRequest
      );
      ...
      this.logger.info("[ToggleNlqQaIgnoreController] Headers:", headers);
      //   2. Check authorization
      const authHeader =
        headers["Authorization"] || headers["authorization"] || "";
      if (!authHeader.startsWith("Bearer ")) {
        this.logger.error(
          "[ToggleNlqQaIgnoreController] No token provided",
          httpRequest
        );
      ...
      this.logger.info("[ToggleNlqQaIgnoreController] Token:", token);

      //   3. Decode token
      const decoded = await this.decodeTokenAdapter.decodeToken(token);
      if (!decoded) {
        this.logger.error(
          "[ToggleNlqQaIgnoreController] Invalid token",
          httpRequest
        );
      ...
      this.logger.info(
        "[ToggleNlqQaIgnoreController] User roles names:",
        roleNames.roleNames
      );
      ...
      if (!hasAuth) {
        this.logger.error(
          "[ToggleNlqQaIgnoreController] User is not authorized"
        );
      ...
      //   ==== INPUT BODY ====
      //   1. Check body
      this.logger.info("[ToggleNlqQaIgnoreController] Body:", httpRequest.body);
      if (!httpRequest.body) {
        this.logger.error("[ToggleNlqQaIgnoreController] No body provided");
      ...
      if (!useCase.success) {
        this.logger.error(
          "[ToggleNlqQaIgnoreController] Error creating NLQ QA",
          {
            ...useCase,
          }
        );
      ...
      this.logger.error(
        "[ToggleNlqQaIgnoreController] Unexpected error",
        err.message
      );
