...
import { IReadAllSchemaUseCase } from "@/core/application/usecases/schema/read-all-schema.usecase";

export class ReadAllSchemaController implements IController {
  constructor(
    private readonly logger: ILogger,
    private readonly readAllSchemaUseCase: IReadAllSchemaUseCase,
  ...
  async handle(httpRequest: IHttpRequest<null>): Promise<IHttpResponse> {
    try {
      // ==== INPUT OF REQUEST ====
      this.logger.info(
        "[ReadAllSchemaController] handling request",
        httpRequest
      );
      ...
      this.logger.info("[ReadAllSchemaController] Headers:", headers);
      //   2. Check authorization
      const authHeader =
        headers["Authorization"] || headers["authorization"] || "";
      if (!authHeader.startsWith("Bearer ")) {
        this.logger.error(
          "[ReadAllSchemaController] No token provided",
          httpRequest
        );
      ...
      this.logger.info("[ReadAllSchemaController] Token:", token);

      //   3. Decode token
      const decoded = await this.decodeTokenAdapter.decodeToken(token);
      if (!decoded) {
        this.logger.error(
          "[ReadAllSchemaController] Invalid token",
          httpRequest
        );
      ...
      this.logger.info(
        "[ReadAllSchemaController] User roles names:",
        roleNames.roleNames
      );
      ...
      if (!hasAuth) {
        this.logger.error("[ReadAllSchemaController] User is not authorized");
        const error = this.httpErrors.error_401("User is not authorized");
        return new HttpResponse(error.statusCode, error.body);
      }

      // ==== BUSINESS LOGIC USE CASES ====
      const useCase = await this.readAllSchemaUseCase.execute();

      if (!useCase.success) {
        this.logger.error("[ReadAllSchemaController] Error reading schemas", {
          ...useCase,
        });
        const error = this.httpErrors.error_400(
          "Error reading schemas: " + useCase.message
        );
        return new HttpResponse(error.statusCode, error.body);
      }
      ...
      this.logger.error("[ReadAllSchemaController] Unexpected error", err);
