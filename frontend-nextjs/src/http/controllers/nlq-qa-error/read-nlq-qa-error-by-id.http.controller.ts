...
import { IReadNlqQaErrorByIdUseCase } from "@/core/application/usecases/nlq/nlq-qa-error/read-nlq-qa-error-by-id.usecase";

export class ReadNlqQaErrorByIdController implements IController {
  constructor(
    private readonly logger: ILogger,
    private readonly readByIdErrorUseCase: IReadNlqQaErrorByIdUseCase,
  ...
  async handle(httpRequest: IHttpRequest<null>): Promise<IHttpResponse> {
    try {
      // ==== INPUT OF REQUEST ====
      this.logger.info(
        "[ReadNlqQaErrorByIdController] Handling request",
        httpRequest
      );
      ...
      this.logger.info("[ReadNlqQaErrorByIdController] Headers:", headers);
      //   2. Check authorization
      const authHeader =
        headers["Authorization"] || headers["authorization"] || "";
      if (!authHeader.startsWith("Bearer ")) {
        this.logger.error(
          "[ReadNlqQaErrorByIdController] Authorization token is missing",
          httpRequest
        );
      ...
      this.logger.info("[ReadNlqQaErrorByIdController] Token:", token);

      //   3. Decode token
      const decoded = await this.decodeTokenAdapter.decodeToken(token);
      if (!decoded) {
        this.logger.error(
          "[ReadNlqQaErrorByIdController] Invalid authorization token",
          httpRequest
        );
      ...
      this.logger.info(
        "[ReadNlqQaErrorByIdController] User roles names:",
        roleNames.roleNames
      );
      ...
      if (!hasAuth) {
        this.logger.error(
          "[ReadNlqQaErrorByIdController] User does not have the required roles",
          httpRequest
        );
      ...
      if (!httpRequest.params || !httpRequest.params.id) {
        this.logger.error(
          "[ReadNlqQaErrorByIdController] No id parameter provided",
          httpRequest
        );
      ...
      this.logger.info(
        "[ReadNlqQaErrorByIdController] Received params:",
        httpRequest.params
      );

      // ==== BUSINESS LOGIC USE CASES ====
      const useCase = await this.readByIdErrorUseCase.execute(
        httpRequest.params.id
      );

      if (!useCase.success) {
        this.logger.error("[ReadNlqQaErrorByIdController] Error retrieving: ", {
          ...useCase,
        });
        const error = this.httpErrors.error_400(
          "Error retrieving: " + useCase.message
        );
        return new HttpResponse(error.statusCode, error.body);
      }
      ...
      this.logger.error(
        "[ReadNlqQaErrorByIdController] Internal server error:",
        error
      );
