...
import { IGenSchemaCtxUseCase } from "@/core/application/usecases/gen-topo/gen-schema-ctx.usecase";
import { TSchemaCtxSimpleSchemaDto } from "@/core/application/dtos/schemaCtx.dto";

export class GenSchemaCtxController implements IController {
  constructor(
    private readonly logger: ILogger,
    private readonly useCase: IGenSchemaCtxUseCase,
  ...
  async handle(
    httpRequest: IHttpRequest<TSchemaCtxSimpleSchemaDto>
  ): Promise<IHttpResponse> {
    try {
      // ==== INPUT OF REQUEST ====
      this.logger.info(
        "[GenerateDetailQuestionController] handling request",
        httpRequest
      );
      ...
      this.logger.info("[GenerateDetailQuestionController] Headers:", headers);
      //   2. Check authorization
      const authHeader =
        headers["Authorization"] || headers["authorization"] || "";
      if (!authHeader.startsWith("Bearer ")) {
        this.logger.error(
          "[GenerateDetailQuestionController] No token provided",
          httpRequest
        );
      ...
      this.logger.info("[GenerateDetailQuestionController] Token:", token);

      //   3. Decode token
      const decoded = await this.decodeTokenAdapter.decodeToken(token);
      if (!decoded) {
        this.logger.error(
          "[GenerateDetailQuestionController] Invalid token",
          httpRequest
        );
      ...
      this.logger.info(
        "[GenerateDetailQuestionController] User roles names:",
        roleNames.roleNames
      );
      ...
      if (!hasAuth) {
        this.logger.error(
          "[GenerateDetailQuestionController] User is not authorized"
        );
      ...
      //   1. Check body
      this.logger.info(
        "[GenerateDetailQuestionController] Body:",
        httpRequest.body
      );
      if (!httpRequest.body) {
        this.logger.error(
          "[GenerateDetailQuestionController] No body provided"
        );
      ...
      if (!useCase.success) {
        this.logger.error(
          "[GenerateDetailQuestionController] Error creating NLQ QA",
          {
            ...useCase,
          }
        );
