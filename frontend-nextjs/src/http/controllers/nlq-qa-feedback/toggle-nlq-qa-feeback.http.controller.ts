...
import { TNlqQaFeedbackInOrqDto } from "@/core/application/dtos/nlq/nlq-qa-feedback.app.dto";
import { IToggleNlqQaFeedbackOrchestrator } from "@/core/application/orquestrator/toggle-nlq-qa-feedback.orquestator";

type body = Omit<TNlqQaFeedbackInOrqDto, "userId">;

export class ToggleNlqQaFeedbackController implements IController {
  constructor(
    private readonly logger: ILogger,
    private readonly nlqFeedbackOrchestrator: IToggleNlqQaFeedbackOrchestrator,
  ...
  async handle(httpRequest: IHttpRequest<body>): Promise<IHttpResponse> {
    try {
      ...
      const useCase = await this.nlqFeedbackOrchestrator.execute({
        feedbackId: body.feedbackId,
        isGood: body.isGood,
        comment: body.comment,
        userId: decoded.uid,
        nlqQaId: body.nlqQaId,
      });
