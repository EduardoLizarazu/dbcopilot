...
export function readAllNlqQaGoodComposer(): IController {
  // PROVIDERS
  ...
  // USE CASES
  const useCase = new ReadAllNlqQaGoodUseCase(loggerProvider, nlqQaGoodRepo);

  // CONTROLLER
  const controller = new ReadAllNlqQaGoodController(
    loggerProvider,
    useCase,
    decodeTokenAdapter,
    authRepository
